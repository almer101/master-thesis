...
% TODO: change all "that is" to have a comma afterwards
...
\centering
	...
	\addplot[thick, dashed, color=violet] table[x index=12, y index=13] {data/gbm_hedging_distributions.dat};
	% \addplot[color=magenta] table {data/jump_diffusion_process_falling.dat};
	% \addlegendentry{$n = 1$}
	% \addlegendentry{$n = 2$}
	\addlegendentry{$n = 4$}
...
% TODO: put second fundamental theorem above in \ref{}
...
% STEP 1:
% we generate 30+ jump-diffusion paths and store them in a csv
% set lambda to be bigger (cca 8) and jump size to be N(mean=0.08, std=0.35)
% columns are
% t  x1  call1  x2  call2 ...

% STEP 2:
% on each of those paths evaluate hedging performance for delta-hedge and quadratic-hedge
% you end up with a table like this
% delta    quadratic    diff(quad-delta)
% 48.8       47.9          -0.9
% etc.....

% STEP 3:
% perform a paired t-test on the sample and see whether the quadratic hedge is better
