...
\end{itemize}
We have mentioned a term $\sigma$-algebra, which a reader may not be familiar with. $\sigma$-algebra on a set $\Omega$ is a collection $\Sigma$ of subsets of $\Omega$ which is closed under complement, union and intersection. What this means is that if certain subsets $A,B\subseteq\Omega$ are in $\Sigma$, then simply also $\overline{A}$, $\overline{B}$, $A\cup B$ and $A\cap B$ have to be elements of $\Sigma$. This has to hold for any two elements of $\Sigma$.
